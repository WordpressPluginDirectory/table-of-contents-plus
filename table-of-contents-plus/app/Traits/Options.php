<?php
namespace AIOSEO\TableOfContents\Traits;
...
trait Options {
			...
			if ( $preserveHtml ) {
				if ( is_array( $defaults[ $name ]['value'] ) ) {
					foreach ( $defaults[ $name ]['value'] as $k => $v ) {
						$defaults[ $name ]['value'][ $k ] = aioseoTableOfContents()->helpers->decodeHtmlEntities( $v );
					}
				} else {
					$defaults[ $name ]['value'] = aioseoTableOfContents()->helpers->decodeHtmlEntities( $defaults[ $name ]['value'] );
				}
			}
	...
	public function __set( $name, $value ) {
		...
		aioseoTableOfContents()->core->optionsCache->setOptions( $this->optionsName, $cachedOptions );
	...
	public function update( $optionsName = null, $defaults = null, $options = null ) {
		...
		aioseoTableOfContents()->core->optionsCache->setDb( $optionsName, $refactored );
	 ...
	 * @param  array                                     $arguments Any arguments needed if this was a method called.
	 * @param  mixed                                     $value     The value if we are setting an option.
	 * @return \AIOSEO\TableOfContents\Options\Options            The options object.
	 */
				...
				if ( $preserveHtml ) {
					if ( is_array( $value['value'] ) ) {
						foreach ( $value['value'] as $k => $v ) {
							$value['value'][ $k ] = aioseoTableOfContents()->helpers->decodeHtmlEntities( $v );
						}
					} else {
						$value['value'] = aioseoTableOfContents()->helpers->decodeHtmlEntities( $value['value'] );
					}
				}
	...
	public function refresh() {
		...
		aioseoTableOfContents()->core->optionsCache->resetDb();
		$this->init();
	}
	...
	public function getDbOptions( $optionsName ) {
		$cache = aioseoTableOfContents()->core->optionsCache->getDb( $optionsName );
		if ( empty( $cache ) ) {
			...
			// Set the cache.
			aioseoTableOfContents()->core->optionsCache->setDb( $optionsName, $options );
		}

		return aioseoTableOfContents()->core->optionsCache->getDb( $optionsName );
	}
	 ...
	 * @since 1.0.0
	 *
	 * @return \AIOSEO\TableOfContents\Options\Options The cloned Options object.
	 */
	...
	public function getOriginalInstance() {
		...
		$optionName = aioseoTableOfContents()->helpers->toCamelCase( $class->getShortName() );

		if ( isset( aioseoTableOfContents()->{ $optionName } ) ) {
			return aioseoTableOfContents()->{ $optionName };
		}
