...
class Model implements \JsonSerializable {
	...
	protected function loadData( $var = null ) {
		...
		$query = aioseoTableOfContents()->core->db
			->start( $this->table )
			->where( $this->pk, $var )
			->limit( 1 )
			->output( 'ARRAY_A' );
	...
	protected function filter( $key ) {
		...
		$table   = aioseoTableOfContents()->core->db->prefix . $this->table;
		$results = aioseoTableOfContents()->core->db->execute( 'SHOW COLUMNS FROM `' . $table . '`', true );
	...
	protected function transform( $data, $set = false ) {
		...
		foreach ( $this->jsonFields as $field ) {
			if ( isset( $data[ $field ] ) && ! aioseoTableOfContents()->helpers->isJsonString( $data[ $field ] ) ) {
				if ( is_array( $data[ $field ] ) && aioseoTableOfContents()->helpers->isArrayNumeric( $data[ $field ] ) ) {
		...
		if ( ! empty( $fields ) ) {
			...
			if ( isset( $this->$pk ) && '' !== $this->$pk ) {
				...
				if ( ! $query->nullSet() ) {
					...
					$id = aioseoTableOfContents()->core->db
						->insert( $this->table )
						->set( $fields )
						->run()
						->insertId();
